    ...
    var wzhAdminName = 'wzh-admin';
    var wzhInterfaceName = 'wzh-interface';
    var wzhGateway = 'wzh-gateway';
    
	window.isLocalhost = window.location.hostname == "localhost";
	window.ctx = "";
	if (window.location.protocol == "file:") {
	    var url = window.location.href.split("/wzh/");
	    if (url.length == 2) {
	        window.ctx = url[0] + "/";
	    }
	}else {
	    window.ctx = window.location.protocol + "//" + window.location.host + "/"+wzhGateway+"/";
	}
	
	window.root = window.ctx + "wzh/";
	...
	window.serverPath = window.server + wzhInterfaceName+"/";
	//后台上传的图片地址
	window.serverUploadPath = window.server + wzhAdminName+"/";
